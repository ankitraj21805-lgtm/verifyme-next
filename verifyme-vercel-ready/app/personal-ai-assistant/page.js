...
export default function PersonalAIAssistantPage() {
  ...
  useEffect(() => {
    try { setTasks(JSON.parse(localStorage.getItem("personal-ai-tasks") || "[]")); } catch {}
    setReady(true);
  }, []);
  ...
  function addBot(text) { setMessages((m) => [...m, { from: "bot", text }]); }

  function openMessageDraft(body) {
  ...
  const css = `.pai-wrap{min-height:100vh;background:#0f1115;color:#e8eaed;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;padding:18px;display:flex;justify-content:center}.pai{width:100%;max-width:480px;background:#171a21;border:1px solid #2a2f3a;border-radius:24px;overflow:hidden;box-shadow:0 24px 80px rgba(0,0,0,.35)}.top{padding:18px;border-bottom:1px solid #2a2f3a;display:flex;align-items:center;justify-content:space-between}.top h1{font-size:18px;margin:0}.pill{font-size:12px;color:#9aa0ab;border:1px solid #2a2f3a;border-radius:20px;padding:6px 10px}.tabs{display:flex;background:#11141a}.tabs button{flex:1;background:transparent;border:0;color:#9aa0ab;padding:12px;font-weight:700}.tabs button.active{color:#4f8cff;border-bottom:2px solid #4f8cff}.body{height:620px;max-height:72vh;display:flex;flex-direction:column;padding:14px}.messages{flex:1;overflow:auto;display:flex;flex-direction:column;gap:10px}.msg{max-width:82%;padding:11px 13px;border-radius:16px;font-size:14px;line-height:1.45}.msg.user{align-self:flex-end;background:#2f6fe0;color:white;border-bottom-right-radius:5px}.msg.bot{align-self:flex-start;background:#1f232c;border-bottom-left-radius:5px}.input{border-top:1px solid #2a2f3a;padding-top:12px;display:flex;gap:8px}.input input{flex:1;background:#1f232c;color:#fff;border:1px solid #2a2f3a;border-radius:22px;padding:12px 14px;outline:none}.input button,.primary{background:#4f8cff;color:white;border:0;border-radius:12px;padding:11px 14px;font-weight:800}.icon{border-radius:50%!important;width:44px}.notice{background:#1f232c;border:1px solid #2a2f3a;border-radius:16px;padding:14px;color:#9aa0ab;font-size:13px;line-height:1.55;margin-bottom:12px}.task{background:#1f232c;border:1px solid #2a2f3a;border-radius:14px;padding:12px;margin-bottom:10px;display:flex;gap:10px;align-items:flex-start}.task span{flex:1}.task.done span{text-decoration:line-through;color:#9aa0ab}.danger{background:#e05a5a!important}.small{font-size:12px;color:#9aa0ab}.empty{text-align:center;margin-top:80px;color:#9aa0ab}.safe{padding:14px;font-size:12px;color:#9aa0ab;border-top:1px solid #2a2f3a;line-height:1.55}`;

  return <main className="pai-wrap"><style>{css}</style><section className="pai"><div className="top"><h1>🤖 Personal AI Assistant</h1><div className="pill">● {status}</div></div><div className="tabs"><button onClick={() => setTab("chat")} className={tab === "chat" ? "active" : ""}>Chat</button><button onClick={() => setTab("tasks")} className={tab === "tasks" ? "active" : ""}>Tasks</button><button onClick={() => setTab("settings")} className={tab === "settings" ? "active" : ""}>Safety</button></div>{tab === "chat" && <div className="body"><div className="notice">Safe mode: app background messages/call logs nahi padhta, aur message/call automatically send nahi karta. Aap final confirm karte ho.</div><div className="messages">{messages.map((m,i)=><div key={i} className={`msg ${m.from}`}>{m.text}</div>)}</div><div className="input"><button className="icon" onClick={startVoice}>🎙️</button><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter') send();}} placeholder="Type command: message, call, remind, plan..."/><button onClick={send}>➤</button></div></div>}{tab === "tasks" && <div className="body"><div className="notice">Local tasks/reminders browser storage me save hote hain.</div>{tasks.length===0 ? <div className="empty">No tasks yet. Try: remind me to apply for jobs</div> : tasks.map(t=><div key={t.id} className={`task ${t.done?'done':''}`}><input type="checkbox" checked={t.done} onChange={()=>setTasks(tasks.map(x=>x.id===t.id?{...x,done:!x.done}:x))}/><span>{t.title}<br/><b className="small">{t.note}</b></span><button onClick={()=>setTasks(tasks.filter(x=>x.id!==t.id))}>🗑</button></div>)}</div>}{tab === "settings" && <div className="body"><div className="notice"><b>Privacy:</b> No hidden SMS, no call log reading, no background sending, no secret recording. Calls open dialer only. Messages open WhatsApp/copy draft only.</div><button className="primary danger" onClick={()=>{localStorage.removeItem('personal-ai-tasks');setTasks([]);}}>Delete all local data</button><p className="small">For full Claude AI response, Vercel environment variable ANTHROPIC_API_KEY must be added in project settings.</p></div>}<div className="safe">Open this link on Android Chrome → three dots → Add to Home screen.</div></section></main>;
}
